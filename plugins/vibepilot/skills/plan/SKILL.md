...
- `.vibepilot/work/requirements.md` must exist with substantive content (real sections beyond `# Requirements` and any `> seed` line). If still a placeholder, tell the user to run `/vibepilot:clarify` first and stop.
...
2. **Plan append/rewrite gate.** If `plan.md` has any `##` section (i.e. content beyond `# Plan`), ask via AskUserQuestion and normalize the answer into the token `refine` or `rewrite` (strip the `(Recommended)` suffix and any prose). Capture as `plan_mode`. If `plan.md` is empty or a bare placeholder, set `plan_mode = rewrite` and skip the question.
   - `Refine existing plan (Recommended)` → `refine`
   - `Rewrite from scratch` → `rewrite`
...
3. **Tasks append/rewrite gate + max-number scan.** Using the `tasks.md` content read in step 1: if it contains any `- [ ]` or `- [x]` entry:
   ...
   - Ask via AskUserQuestion. Capture the answer as `tasks_mode`. Default-recommend the option that matches `plan_mode`:
     - When `plan_mode == "rewrite"`: `Rewrite tasks from scratch (Recommended)` / `Append new tasks at the end`.
     - When `plan_mode == "refine"`: `Rewrite tasks from scratch` / `Append new tasks at the end (Recommended)`.
...
4. **Delegate to `vibe-planner` in draft mode** via the `Agent` tool with `subagent_type: "vibe-planner"`. Include in the parent prompt:
   ...
   - `plan_mode: refine` or `plan_mode: rewrite`
...
6. **Delegate to `vibe-planner` in finalize mode** via the `Agent` tool with `subagent_type: "vibe-planner"`. Include in the parent prompt:
   ...
   - Instruction: write both `.vibepilot/work/plan.md` and `.vibepilot/work/tasks.md`, return one combined summary under 200 words — never paste the plan or task content into the response.
